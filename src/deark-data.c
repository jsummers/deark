...
static const u16 cp866table[128] = {
...
// Encodings for which the first 128 chars are the same as CP437.
static de_rune de_cp437like_to_unicode(struct de_encconv_state *es, i32 a,
	const u16 tbl2[128])
{
	de_rune n;
	...
	else if(a>=0x80 && a<=0xff) n = (de_rune)tbl2[a-0x80];
	else {
	...
	dbuf *outf, unsigned int flags)
{
	...
	struct base64_ctx *d = NULL;

	d = de_malloc(c, sizeof(struct base64_ctx));
	d->cbuf_count = 0;
	while(pos<pos1+len) {
		b = dbuf_getbyte_p(inf, &pos);
		if(b>='A' && b<='Z') {
		...
		if(d->cbuf_count>=4) {
			dbuf_writebyte(outf, (u8)((d->cbuf[0]<<2)|(d->cbuf[1]>>4)));
			dbuf_writebyte(outf, (u8)((d->cbuf[1]<<4)|(d->cbuf[2]>>2)));
			dbuf_writebyte(outf, (u8)((d->cbuf[2]<<6)|d->cbuf[3]));
			d->cbuf_count = 0;
		}
	}
	...
	if(d->cbuf_count>0 && !found_terminator) {
