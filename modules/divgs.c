	...
	i64 bitmap_pos)
{
	...
	de_dbg(c, "bitmap at %"I64_FMT, bitmap_pos);
	if(!de_good_image_dimensions(c, md->w, md->h)) goto done;
	img = de_bitmap_create(c, md->w, md->h, 3);
	de_convert_image_paletted(c->infile, bitmap_pos, 8,
		md->w, d->pal, img, 0);
...
{
	if(!dbuf_memcmp(c->infile, 0, (const void*)"fpg\x1a\x0d\x0a\0", 7)) {
		return 100;
	}
	// TODO: "f16" format?
	return 0;
