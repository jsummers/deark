...
{
	fmtutil_ftcomp_codectype1(md->c, md->dcmpri, md->dcmpro, md->dres, NULL);
}
...
static void do_os2pack_ea(deark *c, de_arch_lctx *d, struct de_arch_member_data *md)
{
	int saved_indent_level;
	dbuf *attr_data = NULL;
	de_module_params *mparams = NULL;
	struct os2pack_md_ctx *mdextra = (struct os2pack_md_ctx*)md->userdata;
	struct de_dfilter_in_params dcmpri;
	struct de_dfilter_out_params dcmpro;
	struct de_dfilter_results dres;

	de_dbg_indent_save(c, &saved_indent_level);
	de_dbg(c, "ext. attr. at %"I64_FMT, mdextra->ea_pos);
	de_dbg_indent(c, 1);
	...
	dcmpri.f = c->infile;
	dcmpri.pos = mdextra->ea_pos;
	dcmpri.len = mdextra->ea_len;
	dcmpro.f = attr_data;
	if(mdextra->is_pack2) {
		dcmpro.len_known = 1;
		dcmpro.expected_len = mdextra->ea_orig_len;
	}
	if(mdextra->is_pack2) {
		fmtutil_ftcomp_codectype1(c, &dcmpri, &dcmpro, &dres, NULL);
	}
	else {
...
static void do_os2pack12_member(deark *c, de_arch_lctx *d, struct de_arch_member_data *md)
{
	...
	mdextra->is_pack2 = (d->fmtcode==0xfffd);
	...
	if(d->fmtcode==0x1400 || d->fmtcode==0x0a14) {
		...
		pos += 3;
		mdextra->ea_pos = de_getu32le_p(&pos);
		de_dbg(c, "ext. attr. pos: %"I64_FMT, mdextra->ea_pos);
	...
	if(mdextra->is_pack2) {
		...
		mdextra->ea_orig_len = de_getu32le_p(&pos);
		de_dbg(c, "ext. attr. decompressed len: %"I64_FMT, mdextra->ea_orig_len);
	}
	...
	if(mdextra->ea_pos != 0) {
		if(mdextra->ea_pos<md->cmpr_pos || mdextra->ea_pos>member_endpos) {
			d->fatalerrflag = 1;
			d->need_errmsg = 1;
			goto done;
		}
		md->cmpr_len = mdextra->ea_pos - md->cmpr_pos;
		mdextra->ea_len = member_endpos - mdextra->ea_pos;
	}
	...
	if(mdextra->ea_len>0) {
		de_dbg(c, "cmpr ext. attr. at %"I64_FMT", len=%"I64_FMT, mdextra->ea_pos, mdextra->ea_len);
		de_dbg_indent(c, 1);
		if(mdextra->is_pack2) {
			os2pack2_read_cmpr_method(c, mdextra->ea_pos, mdextra->ea_len);
		}
		de_dbg_indent(c, -1);
	}
	...
	if(mdextra->is_pack2) {
		...
		// filename field.
		// It has a compression header that we can peek at.
	...
	else if(mdextra->is_pack2 && mdextra->enable_pack2) {
		md->dfn = os2pack2_decompressor_fn;
	}

	if(md->dfn) {
		de_arch_extract_member_file(md);
		if(mdextra->ea_pos>0 && mdextra->ea_len>0) {
			do_os2pack_ea(c, d, md);
		}
	}
...
static void do_run_os2pack12(deark *c, de_module_params *mparams, UI ver)
{
	...
	if(d->fmtcode==0xfffd) {
		...
		if(!enable_pack2) {
			de_warn(c, "Decompression of this format is disabled. \"-opt os2pack2:decode\" to enable.");
		}
	}
	else {
	...
	while(1) {
		...
		mdextra = de_malloc(c, sizeof(struct os2pack_md_ctx));
		mdextra->enable_pack2 = enable_pack2;
		md = de_arch_create_md(c, d);
		md->userdata = (void*)mdextra;
