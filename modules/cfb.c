...
static void thumbsdb_msrgba_write_dqt(deark *c, dbuf *outf)
{
	size_t i;
	u8 tbl[64];

	fmtutil_get_std_jpeg_qtable(0, tbl);

	for(i=0; i<64; i++) {
		tbl[i] = (u8)(((UI)tbl[i]+1)/2);
	}

	dbuf_write(outf, (const u8*)"\xff\xdb\x00\x43\x00", 5);
	dbuf_write(outf, tbl, 64);
}
	...
	i64 hdrsize, dbuf *outf)
{
	...
	// DQT
	thumbsdb_msrgba_write_dqt(c, outf);
	...
	// DHT
	dbuf_write(outf, (const u8*)"\xff\xc4\x00\xd2\x00", 5);
	...
	dbuf *f, i64 pos1, i64 len, int is_thumb)
{
	...
	de_bitmap_write_to_fileOLD(img, is_thumb?"thumb":NULL, DE_CREATEFLAG_FLIP_IMAGE);
